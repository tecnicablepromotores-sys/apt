...
class TecnicableSync(
    ...
    private val auth: FirebaseAuth = FirebaseAuth.getInstance().apply {
        try {
            firebaseAuthSettings.forceRecaptchaFlowForTesting(false)
            firebaseAuthSettings.setAppVerificationDisabledForTesting(true)
        } catch (e: Exception) {
            Log.w("TecnicableSync", "Could not configure Auth settings for testing", e)
        }
    }
) {
    ...
    fun subirClienteApp(contrato: ContratoDiario, promotorId: String = "") = ensureAuth {
        val data = hashMapOf(
            "id" to contrato.id,
            "uuid" to contrato.uuid,
            "nroInstalacion" to contrato.nroInstalacion,
            "nombreCliente" to contrato.nombreCliente,
            "cliente" to contrato.nombreCliente,
            "nombre" to contrato.nombreCliente,
            "cedula" to contrato.cedula,
            "celular" to contrato.celular,
            "telefono" to contrato.celular,
            "correo" to contrato.correo,
            "email" to contrato.correo,
            "fechaNacimiento" to contrato.fechaNacimiento,
            "plan" to contrato.plan,
            "tipoOnu" to contrato.tipoOnu,
            "tipoServicio" to contrato.tipoServicio,
            ...
            "puntoReferencia" to contrato.puntoReferencia,
            "direccion" to contrato.direccion,
            "direccionCompleta" to contrato.direccion,
            "sector" to contrato.direccion,
            "fecha" to contrato.fecha,
            "latitud" to contrato.latitud,
            "longitud" to contrato.longitud,
            "latitudCaja" to contrato.latitudCaja,
            "longitudCaja" to contrato.longitudCaja,
            "firmaUri" to contrato.firmaUri,
            "fotoClientUri" to contrato.fotoClientUri,
            "fotoCajaUri" to contrato.fotoCajaUri,
            "firmaBase64" to contrato.firmaBase64,
            "fotoClientBase64" to contrato.fotoClientBase64,
            "fotoCajaBase64" to contrato.fotoCajaBase64,
            "tecnicoNombre" to contrato.tecnicoNombre,
            "promotor" to contrato.tecnicoNombre,
            "cerrado" to contrato.cerrado,
    ...
    fun subirProspectoCenso(prospecto: ProspectoCenso) = ensureAuth {
        val data = hashMapOf(
            "id" to prospecto.id,
            "nombreCompleto" to prospecto.nombreCompleto,
    ...
    fun loginSeguro(
        ...
        val safeClave = if (clave.length >= 6) clave else clave.padEnd(6, '0')
        if (correo.isBlank() || !correo.contains("@") || safeClave.isBlank()) {
            onResult(false, "Ingrese un correo electrónico y contraseña válidos.", null)
            return
        }

        auth.signInWithEmailAndPassword(correo, safeClave)
...
class FirestoreWebSync(private val firestore: FirebaseFirestore) {
    ...
    fun subirClienteApp(contrato: ContratoDiario, promotorId: String = "") {
        sync.subirClienteApp(contrato, promotorId)
    }
